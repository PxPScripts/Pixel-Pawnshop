...
Citizen.CreateThread(function()
	while true do
		...
		if dist <= 2 then
			DrawText3Ds(Config.PawnShopLocation['x'],Config.PawnShopLocation['y'],Config.PawnShopLocation['z'],"Tryk ~r~[E]~s~ for at tale med ~r~ Jens")
	...
	for k, v in pairs(ESX.GetPlayerData().inventory) do
		...
		if price and v.count > 0 then
			table.insert(elements, {
				label = ('%s - <span style="color:green;">%s</span>'):format(v.label, 'DKK'..price, ESX.Math.GroupDigits(price)),
				name = v.name,
				price = price,
